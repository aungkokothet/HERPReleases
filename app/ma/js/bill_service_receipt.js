...
function saveObj() {
    ...
    var end_point = API_URI + "bill_service_receipt/add";
    var data_send = {};
    ...
    if(isnew) { //inserting new
        ...
        request_type = "POST"
        end_point = API_URI + "bill_service_receipt/" + user_id + '/update';
...
function deleteObj() {
    ...
    end_point = API_URI + "bill_service_receipt/" + user_id + "/remove";
...
function load() {
    var pvar = getPvar();
    var end_point = API_URI + "bill_service_receipt";
    $.ajax({
        url : end_point,
        type: 'POST',
        headers: {'Authorization': 'Bearer '+ pvar.token, "Content-Type" : "application/json"}
