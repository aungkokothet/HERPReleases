...
function clearDataEntryPanel() {
    ...
    $("#data_id, #pharmacy_purchase_id, #pharmacy_item_id, #quantity, #purchase_price, #amount").val('');
...
function saveObj() {
    ...
    var end_point = API_URI + "pharmacy_purchase_items/add";
    var data_send = {};
    ...
    if(isnew) { //inserting new
        ...
        end_point = API_URI + "pharmacy_purchase_items/" + data_send.id + '/update';
...
function deleteObj() {
  console.log(datatable.rows({selected: true}).data()[0])
    ...
    end_point = API_URI + "pharmacy_purchase_items/" + user_id + '/remove';
...
function load() {
    var pvar = getPvar();
    var end_point = API_URI + "pharmacy_purchase_items";
    $.ajax({
        url : end_point,
        type: 'POST',
        headers: {"Authorization":'Bearer '+pvar.token}
...
function loadTable(table_data) {
    var data = table_data.map(x => ({
      ...
      updated: moment(x.updated_time).format('MMM-DD_YYYY')
    }))
