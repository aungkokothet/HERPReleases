...
function clearDataEntryPanel() {
    ...
    $("#data_id, #employee_id, #basic_monthly_rate, #overtime_hourly_rate").val('');
...
function editButtonClick() {
    if(datatable.rows('.selected').any()) {
        ...
        $("#basic_monthly_rate").val(data[0].basic_monthly_rate);
        $("#overtime_hourly_rate").val(data[0].overtime_hourly_rate);
...
function saveObj() {
    ...
    var end_point = API_URI + "salarys/add";
    var data_send = {};

    if(isnew) { //inserting new
        request_type = "POST";
        data_send.employee_id = $("#employee_id").val();
        data_send.basic_monthly_rate = $("#basic_monthly_rate").val();        
        data_send.overtime_hourly_rate = $("#overtime_hourly_rate").val();
        data_send.id = 12//delete this
    }
    else { //editing update
        request_type = "POST"
        data_send = datatable.rows({selected:  true}).data()[0];
        end_point = API_URI + "salarys/" + data_send.id + '/update';
...
function deleteObj() {
  console.log(datatable.rows({selected: true}).data()[0])
    ...
    end_point = API_URI + "salarys/" + user_id + '/remove';
