...
function editButtonClick() {
    if(datatable.rows('.selected').any()) {
        ...
        $("#department_description").val(data[0].description)
...
function saveObj() {
    ...
    var end_point = API_URI + "departments/add";
    var data_send = {};

    if(isnew) { //inserting new
        request_type = "POST";
        data_send.name = $("#department_name").val()
        data_send.description = $("#department_description").val();

    }
    else { //editing update
        request_type = "POST"
        data_send = datatable.rows({selected:  true}).data()[0];
        end_point = API_URI + "departments/" + data_send.id + '/update';
...
function deleteObj() {
  console.log(datatable.rows({selected: true}).data()[0])
    ...
    end_point = API_URI + "departments/" + user_id + '/remove';
...
function load() {
    var pvar = getPvar();
    var end_point = API_URI + "departments";
    $.ajax({
        url : end_point,
        type: 'POST',
        headers: {"Authorization":'Bearer '+pvar.token}
