...
function clearDataEntryPanel() {
    ...
    $("#data_id, #employee_id, #time_in, #time_out").val('');
...
function editButtonClick() {
    if(datatable.rows('.selected').any()) {
        ...
        $("#time_in").val(moment(data[0].time_in).format("YYYY-MM-DDTHH:mm"));
        $("#time_out").val(moment(data[0].time_out).format("YYYY-MM-DDTHH:mm"));
...
function saveObj() {
    ...
    var end_point = API_URI + "attendances/add";
    var data_send = {};

    if(isnew) { //inserting new
        request_type = "POST";
        data_send.employee_id = $("#employee_id").val();
        data_send.time_in = $("#time_in").val();        
        data_send.time_out = $("#time_out").val();
        data_send.id = 12//delete this
    }
    else { //editing update
        request_type = "POST"
        data_send = datatable.rows({selected:  true}).data()[0];
        end_point = API_URI + "attendances/" + data_send.id + '/update';
...
function deleteObj() {
  console.log(datatable.rows({selected: true}).data()[0])
    ...
    end_point = API_URI + "attendances/" + user_id + '/remove';
...
function load() {
    var pvar = getPvar();
    var end_point = API_URI + "attendances";
    $.ajax({
        url : end_point,
        type: 'POST',
        headers: {"Authorization":'Bearer '+pvar.token}
...
function loadTable(table_data) {
    var data = table_data.map(x => ({
      ...x,
      time_in_mod: moment(x.time_in).format('MMM-DD-YYYY, hh:mm'),
      time_out_mod: moment(x.time_out).format('MMM-DD-YYYY, hh:mm'),
