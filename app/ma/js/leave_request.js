...
function clearDataEntryPanel() {
    ...
    $("#data_id, #employee_id, #leave_type_id, #leave_date, #comment, #day_period").val('');
...
function editButtonClick() {
    if(datatable.rows('.selected').any()) {
        ...
        $("#leave_date").val(moment(data[0].leave_date).format('YYYY-MM-DD'));
        $("#comment").val(data[0].comment);
        $("#day_period").val(data[0].day_period);
...
function saveObj() {
    ...
    var end_point = API_URI + "leave_requests/add";
    var data_send = {};

    if(isnew) { //inserting new
        request_type = "POST";
        data_send.employee_id = $("#employee_id").val();
        data_send.leave_type_id = $("#leave_type_id").val();
        data_send.leave_date = $("#leave_date").val();
        data_send.comment = $("#comment").val();
        data_send.day_period = $("#day_period").val();
        ...
        end_point = API_URI + "leave_requests/" + data_send.id + '/update';
...
function deleteObj() {
  console.log(datatable.rows({selected: true}).data()[0])
    ...
    end_point = API_URI + "leave_requests/" + user_id + '/remove';
...
function load() {
    var pvar = getPvar();
    var end_point = API_URI + "leave_requests";
    $.ajax({
        url : end_point,
        type: 'POST',
        headers: {"Authorization":'Bearer '+pvar.token}
...
function loadTable(table_data) {
    var data = table_data.map(x => ({
      ...x,
      leave_date_mod: moment(x.leave_date).format('MMM-DD-YYYY'),
