...
function clearDataEntryPanel() {
    ...
    $("#data_id, #pharmacy_item_id, #quantity, #sale_price, #amount").val('');
    $("#pharmacy_issue_id").val('');
...
function editButtonClick() {
    if(datatable.rows('.selected').any()) {
        ...
        $("#pharmacy_issue_id").val(data[0].pharmacy_issue_id);
...
function saveObj() {
    ...
    var end_point = API_URI + "pharmacy_issue_items/add";
    var data_send = {};

    if(isnew) { //inserting new
        request_type = "POST";
        data_send.pharmacy_issue_id = $("#pharmacy_issue_id").val();
        ...
        end_point = API_URI + "pharmacy_issue_items/" + data_send.id + '/update';
...
function deleteObj() {
  console.log(datatable.rows({selected: true}).data()[0])
    ...
    end_point = API_URI + "pharmacy_issue_items/" + user_id + '/remove';
...
function load() {
    var pvar = getPvar();
    var end_point = API_URI + "pharmacy_issue_items";
    $.ajax({
        url : end_point,
        type: 'POST',
        headers: {"Authorization":'Bearer '+pvar.token}
