...
function clearDataEntryPanel() {
    ...
    $("#data_id, #name, #allowance_days_per_year").val('');
...
function editButtonClick() {
    if(datatable.rows('.selected').any()) {
        ...
        $("#allowance_days_per_year").val(data[0].allowance_days_per_year);
...
function saveObj() {
    ...
    var end_point = API_URI + "leave_types/add";
    var data_send = {};

    if(isnew) { //inserting new
        request_type = "POST";
        data_send.name = $("#name").val();
        data_send.allowance_days_per_year = $("#allowance_days_per_year").val();
        ...
        end_point = API_URI + "leave_types/" + data_send.id + '/update';
...
function deleteObj() {
  console.log(datatable.rows({selected: true}).data()[0])
    ...
    end_point = API_URI + "leave_types/" + user_id + '/remove';
...
function load() {
    var pvar = getPvar();
    var end_point = API_URI + "leave_types";
    $.ajax({
        url : end_point,
        type: 'POST',
        headers: {"Authorization":'Bearer '+pvar.token}
